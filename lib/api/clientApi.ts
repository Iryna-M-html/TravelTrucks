...
//   // Используем глобальную константу и приводим типы
//   let result = [...MOCK_CAMPERS_DATA.items] as unknown as Camper[];

//   // Локальная фильтрация (чтобы сайт реагировал на кнопки)
//   if (filters) {
//     if (filters.location) {
...
//       filters.equipment.forEach((key) => {
//         result = result.filter((item) => (item as any)[key] === true);
//       });
//     }

//     if (filters.transmission) {
//       result = result.filter(
//         (item) => item.transmission === filters.transmission
//       );
//     }
//   }
...
//   // 1. Имитируем задержку сети
//   await new Promise((resolve) => setTimeout(resolve, 300));

//   // 2. Ищем нужный кемпер в глобальной константе
//   // Приводим тип через unknown, чтобы избежать конфликтов с лишними полями в JSON
//   const camper = MOCK_CAMPERS_DATA.items.find(
//     (item) => item.id === id
//   ) as unknown as Camper;

//   // 3. Если кемпер не найден, выбрасываем ошибку (как это сделал бы бэкенд)
//   if (!camper) {
